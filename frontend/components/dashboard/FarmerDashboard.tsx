...
export default function FarmerDashboard({ user }: Props) {
  ...
  const stats = [
    { label: "Active Listings",  value: activeProducts,          icon: Package,    color: "text-green-400",  bg: "bg-green-950/50 border-green-900/50",  trend: "+2 this week" },
    { label: "Total Views",      value: formatNumber(totalViews),icon: Eye,        color: "text-sky-400",    bg: "bg-sky-950/50 border-sky-900/50",      trend: "+12% this month" },
    { label: "Total Orders",     value: totalOrders,             icon: ShoppingCart,color: "text-amber-400", bg: "bg-amber-950/50 border-amber-900/50",  trend: "+5 new" },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
            ...
            <div className="divide-y divide-white/[0.04]">
              {products.slice(0, 6).map((product) => (
                <div key={product.id} className="flex items-center gap-4 p-4 hover:bg-white/[0.02] transition-colors">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-gray-600 text-xs">{getCategoryLabel(product.category)} · {formatPrice(product.price, product.currency)}/{product.unit}</p>
                  </div>
              ...
              {products.length > 6 && (
                <div className="p-4 text-center">
                  <Link href="/dashboard/farmer/products" className="text-green-400 hover:text-green-300 text-sm font-medium transition-colors">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white/[0.03] border border-white/[0.07] rounded-2xl p-5">
            ...
            {!user.kyc_approved && (
              <Link href="/verify" className="mt-4 w-full bg-amber-500/20 hover:bg-amber-500/30 border border-amber-700/40 text-amber-400 text-xs font-bold px-4 py-2.5 rounded-xl transition-colors flex items-center justify-center gap-1.5">

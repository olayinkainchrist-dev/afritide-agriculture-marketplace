...
@router.post("", summary="Create a sourcing request (goes to admin)")
async def create_rfq(
    payload: RFQCreateSchema,
    current_user=Depends(get_current_user),
    db: Session = Depends(get_db),
...
@router.post("/{rfq_id}/quote", summary="Submit quotation to buyer (admin only)")
async def submit_quote(
    ...
    if rfq.status != RFQStatus.OPEN:
        raise HTTPException(status_code=400, detail="RFQ is no longer open")
    ...
    return success_response(
        data=RFQResponseSchema.model_validate(rfq).model_dump(mode="json"),
        message="Quotation sent to buyer",
    )
...
async def accept_quote(
    ...
    return success_response(message="Quotation accepted. Proceed to checkout.")

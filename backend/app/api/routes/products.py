...
async def list_products(
    ...
    if country:
        query = query.filter(Product.country.ilike(f"%{country}%"))
    if state:
    ...
    if is_export_ready is not None:
        query = query.filter(Product.is_export_ready == is_export_ready)
    if is_negotiable is not None:
        query = query.filter(Product.is_negotiable == is_negotiable)
...
async def get_product(
    ...
    data["seller"] = UserPublicSchema.from_orm(product.seller).dict()
...
async def create_product(
    ...
    return success_response(
        data=ProductResponseSchema.from_orm(product).dict(),

...
async def get_commodity_by_name(
    ...
    """Returns all price types (farm-gate, wholesale, retail, export) for a commodity grouped by type."""
...
async def get_commodity_summary(
    ...
    """Returns unique commodity names with their price count for the price board overview."""
    commodities = db.query(CommodityPrice).filter(
        CommodityPrice.is_active == True

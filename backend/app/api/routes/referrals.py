"""
Afritide - Referral & Commission System Routes
"""
...
async def request_withdrawal(
    ...
    for comm in db.query(CommissionTransaction).filter(
        CommissionTransaction.referrer_id == current_user.id,
        CommissionTransaction.status      == "AVAILABLE",
...
def process_order_commission(order_id: uuid.UUID, db: Session):
    try:
        ...
        if not order or not order.platform_fee or order.platform_fee <= 0:
            return
        ...
        if relationship.referrer_id == order.seller_id:
            risk = ReferralRiskEvent(
                user_id     = relationship.referrer_id,
                event_type  = "SELF_DEAL",
                description = f"Referrer is also seller on order {order.id}",
                event_data  = {"order_id": str(order_id)},
            )
            db.add(risk)
            db.commit()
            return
        ...
        rule = get_active_rule(db)
        if not rule or order.total_amount < rule.min_transaction:
            return
...
async def admin_overview(
    ...
    return success_response(data={
        ...
        "total_commissions":     round(db.query(func.sum(CommissionTransaction.commission_amount)).scalar() or 0, 2),
        "pending_commissions":   round(db.query(func.sum(CommissionTransaction.commission_amount)).filter(CommissionTransaction.status.in_(["PENDING","HELD"])).scalar() or 0, 2),
        "available_commissions": round(db.query(func.sum(CommissionTransaction.commission_amount)).filter(CommissionTransaction.status == "AVAILABLE").scalar() or 0, 2),
        "pending_withdrawals":   db.query(ReferralWithdrawal).filter(ReferralWithdrawal.status == "PENDING").count(),
        "total_paid":            round(db.query(func.sum(ReferralWithdrawal.amount)).filter(ReferralWithdrawal.status == "COMPLETED").scalar() or 0, 2),
    })
